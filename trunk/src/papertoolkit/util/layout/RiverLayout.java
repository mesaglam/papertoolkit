package papertoolkit.util.layout;
 ...
 * to how text is added to a word processor (Components flow like a "river". RiverLayout is however
 * much more powerful than FlowLayout: Components added with the add() method generally gets laid
 * out horizontally, but one may add a string before the component being added to specify
...
public class RiverLayout extends FlowLayout implements LayoutManager, java.io.Serializable {
	...
	final Insets totalInsets = new Insets(0, 0, 0, 0);// Dummy values. Set by getInsets()
	...
	protected void adjustAlignment(Component m) {
		if (hasConstraint(m, RiverLayout.LEFT_ALIGN))
			setAlignment(FlowLayout.LEFT);
		else if (hasConstraint(m, RiverLayout.RIGHT_ALIGN))
			setAlignment(FlowLayout.RIGHT);
		else if (hasConstraint(m, RiverLayout.CENTER_ALIGN))
	...
	public void layoutContainer(Container target) {
		...
		synchronized (target.getTreeLock()) {
			...
			if (toVfill != null && moveDownStart == 0) { // Don't move anything if hfill
...
class Ruler {
	...
	private final Vector<Integer> tabs = new Vector<Integer>();
