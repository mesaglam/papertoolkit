...
/**
 * <p>
 * Filters are both similar to and different from EventHandlers. They are similar in the sense that they can
 * be added to regions. When ink is written on a region, the data will be sent to a filter that is attached to
 * that region.
 * </p>
 * <p>
 ...
 */
public abstract class ContentFilter implements ContentFilterListener {
	...
	public ContentFilter() {
		...
		// external listeners can be added too
		addListener(this);
	}

	/**
	 * We listen for our own events. =) Override this to handle your own content.
	 * 
	 * @see edu.stanford.hci.r3.events.ContentFilterListener#contentArrived()
	 */
	public void contentArrived() {
		DebugUtils.println("New Content Arrived!");
	}

	/**
	 * TODO: A design decision here... Should we make every content filter add a new listener object? Or can a
	 * content filter be its own listener? Would having both approaches make sense? Maybe when a content
	 * filter is constructed, it adds itself as a listener?
	 * 
	 * @param handler
	 */
	...
	/**
	 * Consuming this event has no effect. Additionally, filters are handled AFTER regular event handling.
	 * Thus, consuming this event will not do anything, except impact the event handlers that are attached to
	 * this filter.
	 */
